...
const NotificationScreen = () => {
  ...
  // Resize the current location marker
  const resizedCurrentLocationMarker = Image.resolveAssetSource(currentLocationMarker);
  const scaledCurrentLocationMarker = {
    uri: resizedCurrentLocationMarker.uri,
    width: 40, // Desired width
    height: 40, // Desired height
  };

 // Resize the destination marker
 const resizedDestinationMarker = Image.resolveAssetSource(destinationMarker);
 const scaledDestinationMarker = {
   uri: resizedDestinationMarker.uri,
   width: 40, // Desired width
   height: 40, // Desired height
 };
  ...
  useEffect(() => {
    const startLocationTracking = async () => {
      try {
        ...
        // Save the subscription to state so we can stop it later
    ...
    // Cleanup function to stop location tracking when the component unmounts
    return () => {
  ...
  // Update driver location in Firebase whenever the location changes
  useEffect(() => {
    const updateDriverLocation = async () => {
      if (!phoneNumber || !location) return;

      try {
        // Query the deliverydriver collection for the matching phoneNumber
        const driversQuery = query(collection(db, "deliverydriver"), where("phoneNumber", "==", phoneNumber));
        const querySnapshot = await getDocs(driversQuery);

        if (!querySnapshot.empty) {
          // Get the first matching document (assuming phoneNumber is unique)
          ...
          console.log("Location updated in Firebase:", location);
        } else {
          Alert.alert("Error", "No matching driver found in Firebase.");
        }
      } catch (error: any) {
        Alert.alert("Error", `Failed to update location in Firebase: ${error.message}`);
      }
    };
    ...
    <View style={styles.container}>
      ...
      {Platform.OS === "web" ? (
        ...
        <MapView
          ...
          ref={mapRef}
        >
          {/* <Marker coordinate={location} icon={scaledCurrentLocationMarker} /> */}
